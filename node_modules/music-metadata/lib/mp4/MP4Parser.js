...
export class MP4Parser extends BasicParser {
    constructor() {
        ...
        this.atomParsers = {
             ...
             */
            mvhd: async (len) => {
                const mvhd = await this.readToken(new AtomToken.MvhdAtom(len));
            ...
            ftyp: async (len) => {
                ...
                const types = [];
                while (len > 0) {
                    const ftype = await this.readToken(AtomToken.ftyp);
             ...
             */
            stsd: async (len) => {
                const stsd = await this.readToken(new AtomToken.StsdAtom(len));
             ...
             */
            stsz: async (len) => {
                const stsz = await this.readToken(new AtomToken.StszAtom(len));
                const td = this.getTrackDescription();
    ...
    parseMetadataItemData(metaAtom) {
        ...
        return metaAtom.readAtoms(this.tokenizer, async (child) => {
            ...
            switch (child.header.name) {
                ...
                case 'mean':
                case 'rate': {
                    const name = await this.readToken(new AtomToken.NameAtom(payLoadLength));
                    tagKey += `:${name.name}`;
                    break;
                }
                default: {
                    const uint8Array = await this.readToken(new Token.Uint8ArrayType(payLoadLength));
    ...
    async parseTrackBox(trakBox) {
        ...
        await trakBox.readAtoms(this.tokenizer, async (child) => {
            const payLoadLength = child.getPayloadLength();
            switch (child.header.name) {
                case 'chap': {
    ...
    parseTrackFragmentBox(trafBox) {
        let tfhd;
        return trafBox.readAtoms(this.tokenizer, async (child) => {
            const payLoadLength = child.getPayloadLength();
            switch (child.header.name) {
                case 'tfhd': {
